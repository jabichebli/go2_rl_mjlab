...
def get_arm_target_random(step: int, env_id: int = 0, show_curriculum: bool = False):
    ...
    SEGMENT_DURATION = 1750  # Fixed midpoint
    ...
    # Segment tracking - FIXED duration prevents teleportation
    segment_idx = adjusted_step // SEGMENT_DURATION
    
    # Per-segment speed variation (0.85 to 1.15x)
    ...
    effective_transition = max(FASTEST_TRANSITION, min(SLOWEST_TRANSITION, effective_transition))
    
    # Progress within segment
    steps_in_segment = adjusted_step % SEGMENT_DURATION
    progress = steps_in_segment / float(effective_transition)
